...
{
    private readonly Order _order111;
    ...
    public void OneOrderWithOneProductNoSize()
    {
        ...
        var order111Json =
            JsonOrder111WithProduct("{\"code\": \"Pot\", \"color\": \"red\", \"price\": 16,5, \"currency\": \"SGD\"}");
        Assert.Equal("{\"orders\": [" + order111Json + "]}", new OrdersWriter(_orders).GetContents());
    }
    ...
    public void TwoOrders()
    {
        ...
        Assert.Equal("{\"orders\": [" + order111Json + ", " + order222Json + "]}",
            new OrdersWriter(_orders).GetContents());
    }
